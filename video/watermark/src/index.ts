 ...
 * carries 75 bits. Resolve the real metadata from `templateId` yourself.
 *
 * ```ts
 * const wm = await Watermarker.create({ cacheDir: '.models' })
 * const marked = await wm.embedFrame(frame, { templateId: 42 })
...
export type { Payload, PayloadLayout } from './payload'
export { TEMPLATE_ID_BITS, layoutFor, maxValue } from './payload'
